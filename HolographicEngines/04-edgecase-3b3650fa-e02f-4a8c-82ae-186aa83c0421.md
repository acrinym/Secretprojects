...
- âœ… Critical scenarios (ranked by severity)
- âœ… Probability for each scenario
- âœ… Impact assessment
- âœ… Timeline to recognize/unfold
- âœ… Mitigation for each scenario (prevent/detect/recover)
- âœ… Early warning signs
- âœ… Assumption stress-testing
- âœ… Black swan factors (unknowns)
...
**Oracle â†’ Edge Cases**: Oracle perspectives are stress-tested  
**Mechanical â†’ Edge Cases**: Mechanical dependencies are tested for failure  
**Edge Cases â†’ Tribunal**: Tribunal adjusts verdict based on risk tolerance  
**Edge Cases â†’ Synthesis**: Synthesis includes risk mitigation in action plan  
